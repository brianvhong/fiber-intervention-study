...
plate1 <- lapply(ranges, function(x){
    ...
    mutate(Sample = layout1,
        ...
        summarize(LCAT.ratio = mean(LCAT.ratio),
                  norm.LCAT.ratio = mean(LCAT.ratio))
...
```
 ...
 > Results showed: (fiber.post-fiber.pre)-(placebo.post-placebo.pre)=0.00817, p-value = 0.5689\
...
ggplot(plate.merge, aes(timepoint, LCAT.ratio))+
    geom_boxplot()+
